...
fn bootrom_read() {
    ...
    assert_eq!(status.cycles, 2, "CPU was expected to run {} cycles, {} cycles run instead", 2, status.cycles);
...
fn bootrom_write() {
    ...
    match NativeException::decode_parts(ex.code, ex.associated) {
        ...
        Ok(NativeException::HardwareException(hw_ex)) => panic!("Wrong hardware exception occurred while writing BootROM: {}", hw_ex),
        Ok(ex) => panic!("Expected hardware exception while writing BootROM, got non-hardware exception: {}", ex),
