...
pub fn re_assemble(source: &str) -> String {
    ...
    let assembled = assemble(source).unwrap_or_else(|err| panic!("Failed to assemble source program: {}", err));

    println!("> Decoding assembled program...");

    let decoded = Program::decode(assembled).unwrap_or_else(
        |(instr, err)| panic!("Failed to decode assembled instruction {}: {}", instr + 1, err)
    );
